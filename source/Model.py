...
class Model:
    ...
    def update_song(self):
        ...
        for file_name in files:
            if os.path.splitext(file_name)[1] in Model.Type_of_file:
                source_file = os.path.join(source_dir, file_name)
                if os.path.isfile(source_file):
                    shutil.move(source_file, self.path)

        # Ghi vào CSDL
        add = '''INSERT INTO song (id_song,name_song,singer,date_download,love,listen,type_of_file)
                VALUES (?,?,?,?,?,?,?)'''
        ...
        for root,dirs,files in os.walk(self.path):
            for file in files:
                if os.path.splitext(file)[1] in Model.Type_of_file:
                    ...
                    try:
                        ...
                        value.append(os.path.splitext(file)[0])
                    try:
                    ...
                    value.append(0)
                    value.append(0)
                    value.append(str(os.path.splitext(file)[1]).upper())
                    try:
