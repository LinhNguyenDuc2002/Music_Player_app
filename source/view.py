...
class View(Frame):
    ...
    def init_frame_main(self):
        ...
        # Playlist
        colums = ('name_song','singer','type_of_file','date','listener')
        self.play_list = ttk.Treeview(self.list,
                                      columns=colums,
                                      show='headings',
                                      yscrollcommand=scroll.set,
                                      cursor='hand2')
        self.play_list.column('name_song',width=280)
        self.play_list.column('singer',width=250)
        self.play_list.column('date',width=80)
        self.play_list.column('listener',width=80)
        self.play_list.column('type_of_file',width=70)
        ...
        self.play_list.heading('type_of_file',
                               text='Item type',
    ...
    def play_and_show_song(self,a):
        ...
        try:
            ...
            try:
                ...
                music_data = soundfile.SoundFile(View.song_now)
                self.music_length = len(music_data)/music_data.samplerate
    ...
    def option_selected(self, *args):
        ...
        if a == 'A->Z':
            ...
            View.current_list.sort(key=lambda x : (x[3][6:],x[3][3:5],x[3][0:2]))
            for i in View.current_list:
                value = [i[1],i[2],i[6],i[3],i[5]]
                self.play_list.insert('',END,values=value)
    ...
    def click_listener(self):
        ...
        self.sort_ds()
    def click_type(self):
        View.check_header_playlist = 'type'
        if View.a[4] == 1:
            View.a[4] = 0
        else:
            View.a[4] = 1
    ...
    def sort_ds(self):
        if View.check_header_playlist=='name':
            ...
            if View.a[2] == 0:
                ...
                View.current_list.sort(key=lambda x : (x[3][6:],x[3][3:5],x[3][0:2]),reverse=True)
        elif View.check_header_playlist=='listener':
            if View.a[3] == 0:
                View.current_list.sort(key=lambda x : x[5])
            else:
                View.current_list.sort(key=lambda x : x[5],reverse=True)
        elif View.check_header_playlist=='type':
            if View.a[4] == 0:
                View.current_list.sort(key=lambda x : x[6])
            else:
                View.current_list.sort(key=lambda x : x[6],reverse=True)
        self.play_list.delete(*self.play_list.get_children())
        for i in View.current_list:
            value = [i[1],i[2],i[6],i[3],i[5]]
            self.play_list.insert('',END,values=value)
